...
import { addMetaData } from 'src/app/pages/task-add/task-add.page';
import { accessOfTaskAndFac } from 'src/app/pages/task-detail/task-detail.page';
import { updateShedule, orderStatus } from 'src/app/pages/track-detail/track-detail.page';

@Injectable({
    providedIn: 'root',
})
export class TaskService {
    constructor(public http: HttpClient, public baseData: BaseDataService) {}
    ...
    getAccessbyTaskAndFac(params: getPoParams): Observable<Array<accessOfTaskAndFac>> {
        //验货   （acc）
        return this.baseData.get({
    ...
    uploadTask(data: addMetaData): Observable<any> {
        //上传任务
    ...
    updateSchedule(update: updateShedule) {
        //更新进度
    ...
    getNeedSchedule(contract_id: number): Observable<Array<orderStatus>> {
    ...
    //////////////////////////////////////////////////////////////////////////////// 冗余api
    resetSchedule(contract_id: string): Observable<any> {
        return this.baseData.get({ url: '/schedule/reset_schedule_isNeed', params: { contract_id } });
    }

    multiApplyInspect(params:{sku: string}){
        return this.baseData.get({ url: '/schedule/get_sku_list_by_sku', params})
    }
}
