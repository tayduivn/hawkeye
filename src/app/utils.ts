import { isObject, isNullOrUndefined, isFunction, isString, isArray } from 'util';
...
export function clone(o: any) {
    if (isObject(o)) return JSON.parse(JSON.stringify(o));
    return o;
}
...
export function extJson(o: any, skipNullOrUndef = true): { [prop: string]: any } {
    ...
    function ext(json, prefix, ret) {
        ...
        eachO(json, (v, k) => {
            if (skipNullOrUndef && (isNullOrUndefined(v) || '' === v)) return true;
...
export function eachO<T>(o: object, f: (v: T, k: string | any) => boolean | any) {
    for (const k in o) if (false === f(o[k], k)) break;
}
...
export function eachA<T>(a: Array<T>, f?: ((e: T, i?: number) => boolean) | void | any) {
    ...
    for (let i = 0, len = a.length; i < len; i++) if (false === f(a[i], i)) break;
}
